  ...
  return (
    <section className="space-y-6">
      <div className="rounded-[28px] border border-stone-200 bg-white/85 p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-stone-500">게임을 선택해 등록된 UI 스크린샷을 이미지 중심으로 확인합니다.</p>
      </div>
          ...
          {games.map((game) => {
            ...
            return (
              <button
                ...
                className="group overflow-hidden rounded-[28px] border border-stone-200 bg-white text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
              >
                <div className="flex min-h-64 items-center justify-center bg-[#07080a] p-3">
                  {coverUrl ? (
                    <img
                      src={coverUrl}
                      alt={game.name}
                      className="max-h-[360px] w-full rounded-2xl object-contain transition duration-300 group-hover:scale-[1.015]"
                    />
                  ) : (
