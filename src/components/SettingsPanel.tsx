...
export default function SettingsPanel({
  ...
  async function handleDeleteGame(game: Game) {
    ...
    try {
      ...
      await onRefreshGames();
      if (managedGameId === game.id) {
        setManagedGameId('');
        setManagedScreenshots([]);
      }
    } catch (error) {
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-[28px] border border-stone-200 bg-white/85 p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-stone-500">이미지 업로드, 업로드 이미지 삭제, 게임 등록, 기능 프리셋을 관리합니다.</p>
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <TabButton active={activeTab === 'features'} onClick={() => setActiveTab('features')} icon={<Settings className="h-4 w-4" />} label="기능 관리" />
      ...
      {activeTab === 'features' && (
        <div className="rounded-[28px] border border-stone-200 bg-white/85 p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-black text-stone-950">기능 관리</h3>
              <p className="mt-1 text-sm text-stone-500">업로드와 기능별 보기에서 사용할 기능 프리셋을 관리합니다.</p>
            </div>
            <button
              type="button"
              onClick={() => {
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {draftFeatures.map((feature) => (
              <span key={feature} className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1.5 text-sm font-bold text-stone-700 ring-1 ring-stone-200">
                {feature}
                {editUnlocked && (
                  <button type="button" onClick={() => setDraftFeatures(draftFeatures.filter((item) => item !== feature))} className="text-red-600" aria-label={`${feature} 삭제`}>
